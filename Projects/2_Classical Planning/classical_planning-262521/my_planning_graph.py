...
class ActionLayer(BaseActionLayer):
    ...
    def _inconsistent_effects(self, actionA, actionB):
        ...
        #     if ~b in self.children[actionA]:
        #         return True
        return any(~a in self.children[actionB] or ~b in self.children[actionA]
                   for a in self.children[actionA] for b in self.children[actionB])
    ...
    def _interference(self, actionA, actionB):
        ...
        layers.ActionNode
        """
        # for a_eff in self.children[actionA]:
        #     if ~a_eff in self.parents[actionB]:
        #         return True
        # for b_eff in self.children[actionB]:
        #     if ~b_eff in self.parents[actionA]:
        #         return True
        # return False
        return any(~a_eff in self.parents[actionB] or ~b_eff in self.parents[actionA]
                   for a_eff in self.children[actionA] for b_eff in self.children[actionB])
    ...
    def _competing_needs(self, actionA, actionB):
        ...
        """
        return any(self.parent_layer.is_mutex(a_pre, b_pre) for a_pre in self.parents[actionA] for b_pre in self.parents[actionB])
...
class LiteralLayer(BaseLiteralLayer):
    ...
    def _inconsistent_support(self, literalA, literalB):
        ...
        layers.BaseLayer.parent_layer
        """
        return all(self.parent_layer.is_mutex(actA, actB) and self.parent_layer.is_mutex(actB, actA)
                   for actA in self.parents[literalA] for actB in self.parents[literalB])
...
class PlanningGraph:
    ...
    def h_levelsum(self):
        ...
        costs = []
        graph = self.fill()
        for goal in graph.goal:
            costs.append(self.level_cost(graph, goal))
        return sum(costs)
    ...
    def h_setlevel(self):
        ...
        for i, layer in enumerate(graph.literal_layers):
            ...
            if not allGoalsMet:
                continue
            goalsAreMutex = False
            for goalA in graph.goal:
                for goalB in graph.goal:
                    if goalA == goalB:
                        continue
